...
let quot a b =
  if b = 0
  then invalid_arg "builtin_quot: You can't divide by 0"
  else if b < 0
  then invalid_arg "builtin_quot: b must be a natural number"
  else match sign a with
       |(-1) -> if a mod b = 0
                then a/b
                else a/b - 1
       |_ -> a/b;;
...
let modulo a b =
  ...
  then invalid_arg "builtin_quot: b must be a nature number"
  else if sign a = -1 && a mod b <> 0
  then (a mod b) + b
  else a mod b;;
