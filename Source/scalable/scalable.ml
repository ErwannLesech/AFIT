...
let from_int x =
  if x = 0
  then []
  else let rec from_int_rec x = match (x / 2)  with
            |0 -> (x mod 2)::[]
...
let to_int bA =
  if bA = []
  then 0
  else let rec to_int_rec bA i = match bA with
         |e::b when b = [] -> 0
         |e::b -> e * (2*i) + to_int_rec b (i+1)
       in to_int_rec bA 0;;
...
(** Prints bitarray as binary number on standard output.
    @param bA a bitarray.
  *)
let print_b bA = if bA = []
                 then "0"
                 else let rec print_b_rec bA signedBit = match bA with
                        |[] -> ""
                        |e::b when signedBit -> if e = 1
                                                then "-" ^ print_b_rec b false
                                                else print_b_rec bA false
                        |e::b -> print_b_rec b false ^ string_of_int(e)
                      in print_b_rec bA true;;
...
let rec compare_n nA nB = if (to_int nA) >= (to_int nB)
                          then 1
                          else (-1)
...
(** Absolute value of bitarray.
    @param bA Bitarray.
*)
et abs_b bA = match bA with
  |[] -> invalid_arg "abs_b: bA is empty"
  |e::b -> 0::b;;

(** Quotient of integers smaller than 4 by 2.
    @param a Built-in integer smaller than 4.
*)
let _quot_t a = if a < 2
                then 0
                else 1
